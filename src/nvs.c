...
#define _nvs_LOG_DBG(format, ...) printf("D: " format "\r\n", ##__VA_ARGS__)
#define _nvs_LOG_INF(format, ...) printf("I: " format "\r\n", ##__VA_ARGS__)
#define _nvs_LOG_ERR(format, ...) printf("E: " format "\r\n", ##__VA_ARGS__)
			     ...
			     size_t len)
{
	...
	if (blen > 0) {
		if(fs->ops.write == NULL)
			return -1;
		rc = fs->ops.write(offset, data8, blen);
		if (rc) {
			/* flash write error */
			goto end;
		}
			 ...
			 size_t len)
{
	...
	if(fs->ops.read == NULL)
		return -1;
	rc = fs->ops.read(offset, data, len);
	return rc;
}
...
static int nvs_flash_erase_sector(struct nvs_fs *fs, uint32_t addr)
{
	...
	_nvs_LOG_DBG("Erasing flash at %lx, len %d", (long int) offset,
		fs->sector_size);

	if(fs->ops.erase == NULL)
		return -1;
	rc = fs->ops.erase(offset, fs->sector_size);
...
static int nvs_recover_last_ate(struct nvs_fs *fs, uint32_t *addr)
{
	...
	_nvs_LOG_DBG("Recovering last ate from sector %d",
...
static int nvs_add_gc_done_ate(struct nvs_fs *fs)
{
	...
	_nvs_LOG_DBG("Adding gc done ate at %x", fs->ate_wra & ADDR_OFFS_MASK);
...
static int nvs_gc(struct nvs_fs *fs)
{
	...
	do {
		...
		if ((wlk_prev_addr == gc_prev_addr) && gc_ate.len) {
			/* copy needed */
			_nvs_LOG_DBG("Moving %d, len %d", gc_ate.id, gc_ate.len);
...
static int nvs_startup(struct nvs_fs *fs)
{
	...
	if (rc) {
		...
		if (gc_done_marker) {
			...
			_nvs_LOG_INF("GC Done marker found");
			addr = fs->ate_wra & ADDR_SECT_MASK;
			nvs_sector_advance(fs, &addr);
			rc = nvs_flash_erase_sector(fs, addr);
			goto end;
		}
		_nvs_LOG_INF("No GC Done marker found: restarting gc");
...
int nvs_clear(struct nvs_fs *fs)
{
	...
	if (!fs->ready) {
		_nvs_LOG_ERR("NVS not initialized");
		return -EACCES;
	}
...
int nvs_mount(struct nvs_fs *fs)
{
	...
	if(fs->ops.mutex_init != NULL)
		fs->ops.mutex_init();

	/* check that the write block size is supported */
	if (fs->write_block_size > NVS_BLOCK_SIZE || fs->write_block_size == 0) {
		_nvs_LOG_ERR("Unsupported write block size");
		return -EINVAL;
	}

	/* check the number of sectors, it should be at least 2 */
	if (fs->sector_count < 2) {
		_nvs_LOG_ERR("Configuration error - sector count");
		return -EINVAL;
	}
	...
	_nvs_LOG_INF("%d Sectors of %d bytes", fs->sector_count, fs->sector_size);
	_nvs_LOG_INF("alloc wra: %d, %x",
		(fs->ate_wra >> ADDR_SECT_SHIFT),
		(fs->ate_wra & ADDR_OFFS_MASK));
	_nvs_LOG_INF("data wra: %d, %x",
